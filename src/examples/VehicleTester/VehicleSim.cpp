...
/// unit vector of suspension travel. It points from the axle to the contact
/// patch
...
void VehicleSim::initPhysics() {
  ...
  btCollisionShape *chassisShape = new btBoxShape(
      btVector3(0.5f * bodyWidth, 0.5f * bodyHeight, 0.5f * bodyLength));
  ...
  m_vehicleChassis =
      localCreateRigidBody(vehicleMass, tr, compound); // chassisShape);

  if (renderWheelsAsBoxes) {
    m_tireShape =
        new btBoxShape(btVector3(0.5f * tireWidth, tireRadius, tireRadius));
  } else {
    m_tireShape = new btCylinderShapeX(
        btVector3(0.5f * tireWidth, tireRadius, tireRadius));
  }
  ...
  for (int i = 0; i < 4; i++) {
    m_tireRenderInstances[i] = m_guiHelper->registerGraphicsInstance(
        wheelGraphicsIndex, position, quaternion, tireColor, scaling);
  }

  createVehicle();

  m_guiHelper->createCollisionShapeGraphicsObject(
      m_vehicleChassis->getCollisionShape());
  m_guiHelper->createCollisionObjectGraphicsObject(m_vehicleChassis,
                                                   chassisColor);
}
...
void VehicleSim::addTiresToVehicle() {
  ...
  btVector3 leftFrontTirePositionLocal(0.5f * wheelBaseFront, tireHeight,
                                       halfBodyLength - tireRadius);
  m_vehicle->addTire(leftFrontTirePositionLocal, tireAxleDirLocal,
                     tireSuspensionDirLocal, wheelFriction, tireWidth,
                     tireRadius);

  btVector3 rightFrontTirePositionLocal(-0.5f * wheelBaseFront, tireHeight,
                                        halfBodyLength - tireRadius);
  m_vehicle->addTire(rightFrontTirePositionLocal, tireAxleDirLocal,
                     tireSuspensionDirLocal, wheelFriction, tireWidth,
                     tireRadius);

  btVector3 leftRearTirePositionLocal(0.5f * wheelBaseFront, tireHeight,
                                      tireRadius - halfBodyLength);
  m_vehicle->addTire(leftRearTirePositionLocal, tireAxleDirLocal,
                     tireSuspensionDirLocal, wheelFriction, tireWidth,
                     tireRadius);

  btVector3 rightRearTirePositionLocal(-0.5f * wheelBaseFront, tireHeight,
                                       tireRadius - halfBodyLength);
  m_vehicle->addTire(rightRearTirePositionLocal, tireAxleDirLocal,
                     tireSuspensionDirLocal, wheelFriction, tireWidth,
                     tireRadius);
}

void VehicleSim::initGroundTerrain() {
  btBoxShape *ground = new btBoxShape(btVector3(
      0.5f * terrainExtent, 0.5f * terrainThickness, 0.5f * terrainExtent));
  ...
  m_guiHelper->createCollisionObjectGraphicsObject(groundRigidBody,
                                                   terrainColor);
}
...
void VehicleSim::renderScene() {
  ...
  for (int i = 0; i < m_vehicle->getNumTires(); i++) {
    ...
    //DEBUG CODE HACKKK MDODD TODO REMOVE
    if(i == 0)
    {
      m_vehicle->castRay(const_cast<Tire &>(m_vehicle->getTire(i)));
    }
  }
...
bool VehicleSim::mouseMoveCallback(float x, float y) { return true; }

bool VehicleSim::mouseButtonCallback(int button, int state, float x, float y) {
  return true;
}

bool VehicleSim::keyboardCallback(int key, int state) { return true; }

btRigidBody *VehicleSim::localCreateRigidBody(btScalar mass,
...
VehicleSimCreateFunc(struct CommonExampleOptions &options) {
